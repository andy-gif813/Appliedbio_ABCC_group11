...
# 2. Read data
setwd("~/Desktop")
counts <- read.csv("count_matrix_raw.csv", header = TRUE, row.names = 1)
...
# 8. Save results
#write.csv(as.data.frame(sig_genes), "DESeq2_results.csv")
#write.csv(o_sig_genes, "Verified_ORF_results.csv")
...
tpm <- read.csv("count_matrix_tpm.csv", header = TRUE, row.names = 1)
cor_matrix <- cor(tpm, method = "pearson")
#write.csv(cor_matrix, "pearson.csv")
